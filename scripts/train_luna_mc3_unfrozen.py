...
optimizer = torch.optim.AdamW(
    [{"params": backbone_params, "lr": 5e-6}, {"params": fc_params, "lr": 5e-4}],
    weight_decay=5e-2,
)
...
best_f1 = -1.0
CKPT_OUT = Path(
    "/workspace/moe_medical_vision/checkpoints/expert4_luna16_mc3_unfrozen_best.pth"
)
...
for epoch in range(1, 41):
    ...
    print(
        f"[Epoch {epoch:02d} | {epoch_time:.0f}s] Train F1: {f1_score(tr_y, tr_p, average='macro'):.4f} | Val F1: {val_f1:.4f}",
        flush=True,
    )
    if val_f1 > best_f1:
        best_f1 = val_f1
...
print(f"LUNA UNFROZEN FINALIZADO. BEST F1 MACRO: {best_f1:.4f}")
