...
def build_split_map():
    ...
    return mapping
SPLIT_MAP = build_split_map(); study_ids = sorted(SPLIT_MAP.keys())
study_labels = np.array([SPLIT_MAP[sid]['study_label'] for sid in study_ids])
CAND = pd.read_csv(RAW_ROOT/'candidates.csv')

class CandValFast(Dataset):
    def __init__(self):
...
# ---------------- collect study-level probs ----------------
loader3d=DataLoader(LunaFast3DVal(FAST_ROOT), batch_size=16, shuffle=False)
loader2d=DataLoader(Luna2p5DVal(FAST_ROOT), batch_size=16, shuffle=False)
...
p3d=np.array(p3d); p2d=np.array(p2d)

cand_sources = {'none': np.zeros(len(study_ids), dtype=np.float32)}
if mCand is not None:
    loaderCand=DataLoader(CandValFast(), batch_size=16, shuffle=False)
    ...
    sid_to_idx={sid:i for i,sid in enumerate(study_ids)}
    max_arr=np.zeros(len(study_ids), dtype=np.float32)
    top3_arr=np.zeros(len(study_ids), dtype=np.float32)
    top5_arr=np.zeros(len(study_ids), dtype=np.float32)
    for sid,plist in by_series.items():
        arr=np.array(sorted(plist, reverse=True), dtype=np.float32)
        i=sid_to_idx[sid]
        max_arr[i]=arr[0]
        top3_arr[i]=arr[:min(3,len(arr))].mean()
        top5_arr[i]=arr[:min(5,len(arr))].mean()
    cand_sources={'none': np.zeros(len(study_ids), dtype=np.float32), 'cand_max': max_arr, 'cand_top3': top3_arr, 'cand_top5': top5_arr}
...
best=('none', -1, None, None, None)

# 1) weighted probs + candidate source
for cname, pc in cand_sources.items():
    for w3d in np.linspace(0.0,1.0,21):
        for w2d in np.linspace(0.0,1.0-w3d,21):
            wc=1.0-w3d-w2d
            if wc < -1e-9: continue
            probs = w3d*p3d + w2d*p2d + wc*pc
            for thr in np.linspace(0.05,0.95,37):
                pred=(probs>=thr).astype(int)
                f1=f1_score(study_labels,pred,average='macro')
                if f1>best[1]:
                    best=('weighted_probs', f1, cname, (w3d,w2d,wc), thr)

# 2) weighted logits + candidate source
for cname, pc in cand_sources.items():
    l3=logit(p3d); l2=logit(p2d); lc=logit(np.clip(pc,0.001,0.999)) if cname!='none' else np.zeros_like(p3d)
    for w3d in np.linspace(0.0,1.0,21):
        for w2d in np.linspace(0.0,1.0-w3d,21):
            wc=1.0-w3d-w2d
            if wc < -1e-9: continue
            probs = sigmoid(w3d*l3 + w2d*l2 + wc*lc)
            for thr in np.linspace(0.05,0.95,37):
                pred=(probs>=thr).astype(int)
                f1=f1_score(study_labels,pred,average='macro')
                if f1>best[1]:
                    best=('weighted_logits', f1, cname, (w3d,w2d,wc), thr)

print(f'META_FINAL_BEST mode={best[0]} val_f1={best[1]:.4f} cand={best[2]} weights={best[3]} thr={best[4]:.3f}')
