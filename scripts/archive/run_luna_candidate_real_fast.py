...
CKPT_OUT = CHECKPOINT_DIR / 'expert4_luna16_candidate_REAL_best.pth'
PATCH_SIZE = 32
TARGET_SIZE = 64
...
class LUNACandidateRealDataset(Dataset):
    def __init__(self, split='train', seed=42, neg_pos_ratio=1):
        self.split = split
        split_map = SPLIT_MAPS[split]
        df = CAND[CAND['seriesuid'].isin(split_map.keys())].copy()
        if split == 'train':
            pos = df[df['class'] == 1].copy()
            neg = df[df['class'] == 0].sample(n=min(len(df[df['class']==0]), len(pos)*neg_pos_ratio), random_state=seed)
        ...
        print(f'[LUNA CAND REAL {split}] {len(self.df)} candidates | neg={counts[0]} pos={counts[1]}')
    ...
    def __getitem__(self, idx):
        ...
        x = int(round(idx_xyz[0] * sx))
        y = int(round(idx_xyz[1] * sy))
        z = int(round(idx_xyz[2] * sz))
        x = max(0, min(TARGET_SIZE - 1, x))
        y = max(0, min(TARGET_SIZE - 1, y))
        z = max(0, min(TARGET_SIZE - 1, z))
...
print('='*80)
print('LUNA CANDIDATE REAL FAST — candidate-level train, study-level eval')
print('='*80)
...
for epoch in range(1,19):
    ...
    if row['val_f1'] > best:
        ...
        if wait >= 8:
            ...
            break
print(f"LUNA CANDIDATE REAL FAST: best_val_f1={best:.4f} best_epoch={best_epoch}")
print('DONE')
