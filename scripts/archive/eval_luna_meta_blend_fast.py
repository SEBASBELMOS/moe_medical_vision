...
m2d=resnet18(weights=None); m2d.fc=torch.nn.Sequential(torch.nn.Dropout(0.3), torch.nn.Linear(m2d.fc.in_features,2)); m2d=m2d.to(device)
...
if Path(cand_path).exists():
    ...
    mCand.load_state_dict(torch.load(cand_path, map_location=device, weights_only=False)['model_state_dict']); mCand.eval()
...
cand=np.zeros(len(study_ids), dtype=np.float32)
if mCand is not None:
    loaderCand=DataLoader(CandVal(), batch_size=16, shuffle=False)
    ...
    for sid,plist in by_series.items():
        cand[sid_to_idx[sid]]=max(plist)

best=(-1,None,None)
# focused search around weighted ensemble winner, but include candidate lightly
for w3d in np.linspace(0.2,0.5,13):
    for w2d in np.linspace(0.4,0.8,17):
        wc = 1.0 - w3d - w2d
        ...
        for thr in np.linspace(0.20,0.80,61):
            ...
            if f1>best[0]: best=(f1,(w3d,w2d,wc),thr)
print(f'META_BLEND_FAST_BEST val_f1={best[0]:.4f} weights={best[1]} thr={best[2]:.3f}')
