...
from models.experts_3d import build_luna_expert_v2
...
print('\n' + '='*80)
print('TRAINING EXPERT 4 - LUNA16 (V2)')
print('='*80)
...
model_luna = build_luna_expert_v2(pretrained=True, use_gradient_checkpointing=True).to(device)
criterion_luna = torch.nn.CrossEntropyLoss(
    weight=train_ds_luna.get_class_weights().to(device),
    label_smoothing=0.05,
)
print('luna_v2 sanity:', sanity_check_single_batch(model_luna, train_loader_luna, criterion_luna, device))
optimizer_luna = torch.optim.AdamW(model_luna.parameters(), lr=5e-5, weight_decay=5e-4)
scheduler_luna = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_luna, T_max=20, eta_min=1e-6)
luna_result = fit_3d_expert(
    ...
    patience=8,
    max_grad_norm=1.0,
    checkpoint_path=CHECKPOINT_DIR / 'expert4_luna16_r3d18_v2_best.pth',
)
print('luna_v2_result=', luna_result['best_val_f1'], luna_result['best_epoch'])
print('DONE')
