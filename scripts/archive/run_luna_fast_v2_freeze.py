...
CKPT_OUT = CHECKPOINT_DIR / 'expert4_luna16_FAST_v2_freeze_best.pth'
...
print('PHASE1_FREEZE', flush=True)
opt=torch.optim.AdamW(filter(lambda p:p.requires_grad, model.parameters()), lr=2e-4, weight_decay=1e-3)
sched=torch.optim.lr_scheduler.CosineAnnealingLR(opt,T_max=6,eta_min=1e-6)
res1=fit_3d_expert(model, train_loader, val_loader, criterion, opt, device, epochs=6, checkpoint_path=CKPT_OUT, scheduler=sched, accum_steps=2, mixed_precision=True, patience=6)

for p in model.parameters():
    p.requires_grad=True
print('PHASE2_UNFREEZE', flush=True)
opt=torch.optim.AdamW(model.parameters(), lr=1e-5, weight_decay=1e-3)
sched=torch.optim.lr_scheduler.CosineAnnealingLR(opt,T_max=12,eta_min=1e-7)
res2=fit_3d_expert(model, train_loader, val_loader, criterion, opt, device, epochs=12, checkpoint_path=CKPT_OUT, scheduler=sched, accum_steps=2, mixed_precision=True, patience=8)
print('DONE', res2['best_val_f1'], res2['best_epoch'])
