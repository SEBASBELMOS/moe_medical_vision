...
def raw_path_for_item(exp_id, ds, idx):
    if exp_id==0:
        img_name = ds.df.iloc[idx]['Image Index']
        # search in nih tree
        return next(Path('/workspace/moe_medical_vision/data/raw/nih').rglob(img_name))
    if exp_id==1:
        img_id = ds.df.iloc[idx]['image']
        return Path('/workspace/moe_medical_vision/data/raw/isic/ISIC_2019_Training_Input/ISIC_2019_Training_Input') / (img_id + '.jpg')
    if exp_id==2:
        return ds.samples[idx][0]
    return None

results = {i:{'router_correct':0,'total':0,'y_true':[],'y_pred':[],'classes_pred':set()} for i in range(5)}
print('\nIniciando Test Pequeño APP-aligned (10 por experto)...\n')
...
for f in random.sample(files, min(10, len(files))):
    ...
    with torch.no_grad():
        out, best_idx, _ = model(x_router.to(device), x_3d=x_3d.to(device))
    exp_id=3; results[exp_id]['total']+=1
    if best_idx==exp_id: results[exp_id]['router_correct'] +=1
    pred=int(torch.argmax(out, dim=-1).item())
    results[exp_id]['y_true'].append(y_true); results[exp_id]['y_pred'].append(pred); results[exp_id]['classes_pred'].add(pred)
...
for f in random.sample(files, min(10, len(files))):
    ...
    with torch.no_grad():
        out, best_idx, _ = model(x_router.to(device), x_3d=x_3d.to(device))
    exp_id=4; results[exp_id]['total']+=1
    if best_idx==exp_id: results[exp_id]['router_correct'] +=1
    pred=int(torch.argmax(out, dim=-1).item())
    results[exp_id]['y_true'].append(y_true); results[exp_id]['y_pred'].append(pred); results[exp_id]['classes_pred'].add(pred)
# 2D datasets using raw paths for router and ds tensor for expert
cfg={0:('nih_chestxray','/workspace/moe_medical_vision/data/raw/nih'),1:('isic2019','/workspace/moe_medical_vision/data/raw/isic'),2:('osteoarthritis','/workspace/moe_medical_vision/data/raw/osteoporosis/KLGrade/KLGrade')}
for exp_id,(name,root) in cfg.items():
    ...
    for idx in indices:
        ...
        if x_expert.shape[1]==1: x_expert=x_expert.repeat(1,3,1,1)
        if x_expert.shape[-1] != 224: x_expert=F.interpolate(x_expert, size=(224,224), mode='bilinear', align_corners=False)
        raw_path = raw_path_for_item(exp_id, ds, idx)
        x_router = process_router_2d_image(str(raw_path))
        y_true=item['label'].numpy() if exp_id==0 else item['label']
        with torch.no_grad():
            out, best_idx, _ = model(x_router.to(device), x_3d=None)
            # ensure expert uses x_expert if route points to 2D, else fallback 3D internal won't be used here because 2D hierarchy should stay 2D
            if best_idx in [0,1,2]:
                out = model.experts[best_idx](x_expert.to(device))
...
print('='*80)
print('  RESULTADOS FINALES DEL PIPELINE END-TO-END (50 IMAGENES APP-ALIGNED)')
print('='*80)
total_router=sum(r['router_correct'] for r in results.values()); total_imgs=sum(r['total'] for r in results.values())
