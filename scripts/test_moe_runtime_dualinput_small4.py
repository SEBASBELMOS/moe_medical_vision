...
def run_expert(model, best_idx, x_expert, x_3d=None):
    ...
    if best_idx in [3,4]:
        if x_3d is not None:
            ...
            fallback = expert_input[:, :1, :, :].unsqueeze(2)
...
for exp_id, (name, root) in ds_names.items():
    ...
    for idx in indices:
        ...
        if raw_path is None:
            if exp_id == 0 and hasattr(ds, 'df'):
                raw_path = Path('/workspace/moe_medical_vision/data/raw/nih').rglob(ds.df.iloc[idx]['Image Index']).__next__()
            elif exp_id == 1 and hasattr(ds, 'df'):
                raw_path = Path('/workspace/moe_medical_vision/data/raw/isic/ISIC_2019_Training_Input/ISIC_2019_Training_Input') / (ds.df.iloc[idx]['image'] + '.jpg')
