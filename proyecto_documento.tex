...
router. Se calibró $\alpha = 0.20$ tras sweep sobre $\{0.05, 0.10, 0.20\}$:
valores menores subutilizaban la señal de balance en presencia de
desbalance natural fuerte y permitían ratios $> 1.30$ transitorios.
...
+ \mathcal{L}_{\text{aux}}$ sobre embeddings pre-computados, estratificados
por experto (1\,000 muestras por clase en train, 200 en val; AdamW
$\text{lr}=10^{-4}$, weight decay $10^{-4}$, 20 épocas, batch 256,
selección de checkpoint por acc balanceada con desempate por ratio). El
ratio de carga se midió sobre el conjunto balanceado completo al final de
cada época para eliminar varianza de muestreo por mini-batch.
...
Convergencia final: \textbf{Routing Accuracy = 99.60\%} (val balanceado)
$/$ \textbf{98.27\%} (val natural), ratio $\max/\min = \mathbf{1.18}$
sobre el conjunto balanceado — sin colapso y con margen respecto al
umbral de 1.30 exigido por la rúbrica. La distribución en val respeta la
prevalencia natural de cada modalidad: NIH 76.8\%, ISIC 17.4\%, OA 4.9\%,
LUNA16 0.8\%.
