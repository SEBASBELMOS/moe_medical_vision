...
def build_balanced_indices(y, classes, per_class=1500, seed=42):
...
def main():
    ...
    class_ids=[0,1,2]
    idx_tr=build_balanced_indices(y_train, class_ids, per_class=2000, seed=42)
    ...
    model=LinearHead(X_tr.shape[1], 3).to(DEVICE)
    ...
    for epoch in range(1,41):
        ...
        cm=confusion_matrix(y_va_local.numpy(), pred, labels=[0,1,2]).tolist()
    ...
    ckpt=ROOT/'checkpoints'/'router_2d_linear_v2.pth'
    ...
    (ROOT/'metrics'/'router_2d_linear_v2.json').write_text(json.dumps(hist, indent=2))
